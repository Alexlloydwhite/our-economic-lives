...
router.get('/buildingBlocks/:tier/:pyramid', rejectUnauthenticated, (req, res) => {
  ...
  FROM building_block
    ...
    ON industry_pyramid_building_block.building_block_id = building_block.id
	JOIN industry_pyramid 
    ON industry_pyramid.id = industry_pyramid_building_block.industry_pyramid_id
	WHERE building_block.tier_id = $1 
    AND industry_pyramid.id = $2
